...
using Cavex.Principal.Core.Specifications.EmpCatNacionalidad;
...
namespace Cavex.Principal.API.Controllers
{
    ...
    {
        private const string CachePrefix = "EmpCatNacionalidad";
        private readonly IGenericRepository<EmpCatNacionalidad> _repository;
        private readonly IMapper _mapper;
        private readonly IMemoryCache _cache;
        private static int _listCacheVersion = 1;

        public EmpCatNacionalidadController(
            IGenericRepository<EmpCatNacionalidad> repository,
            IMapper mapper,
            IMemoryCache cache)
        {
        ...
        public async Task<ActionResult<ResponseWrapper<PagedResponse<EmpCatNacionalidadDto>>>> GetAll([FromQuery] Pagination pagination)
        {
            ...
            if (_cache.TryGetValue(cacheKey, out ResponseWrapper<PagedResponse<EmpCatNacionalidadDto>>? cachedResponse))
                return Ok(cachedResponse);

            var specification = new EmpCatNacionalidadSpecification(pagination.Search, pagination.PageIndex, pagination.PageSize);
            var countSpecification = new EmpCatNacionalidadCountSpecification(pagination.Search);

            var entities = await _repository.ListAsync(specification);
            var totalCount = await _repository.CountAsync(countSpecification);

            var response = new ResponseWrapper<PagedResponse<EmpCatNacionalidadDto>>
            {
                StatusCode = HttpStatusCode.OK,
                Message = "Consulta realizada correctamente.",
                Data = new PagedResponse<EmpCatNacionalidadDto>
                {
                    PageIndex = pagination.PageIndex,
                    PageSize = pagination.PageSize,
                    TotalCount = totalCount,
                    Items = _mapper.Map<IReadOnlyList<EmpCatNacionalidadDto>>(entities)
                }
            };
        ...
        {
            var entity = await _repository.GetEntityWithSpec(new EmpCatNacionalidadSpecification(id));

            if (entity is null)
            {
                return NotFound(new ResponseWrapper<EmpCatNacionalidadDto>
                {
                    StatusCode = HttpStatusCode.NotFound,
                    Message = "No se encontro la nacionalidad solicitada.",
                    Data = null
                });
            }

            return Ok(new ResponseWrapper<EmpCatNacionalidadDto>
            {
                StatusCode = HttpStatusCode.OK,
                Message = "Consulta realizada correctamente.",
                Data = _mapper.Map<EmpCatNacionalidadDto>(entity)
            });
        }
        ...
        public async Task<ActionResult<ResponseWrapper<EmpCatNacionalidadDto>>> Create([FromBody] RequestWrapper<EmpCatNacionalidadCreateDto> request)
        {
            ...
            {
                return BadRequest(new ResponseWrapper<EmpCatNacionalidadDto>
                {
            ...
            var entity = _mapper.Map<EmpCatNacionalidad>(request.Body);
            ...
            return CreatedAtAction(nameof(GetById), new { id = entity.Id }, new ResponseWrapper<EmpCatNacionalidadDto>
            {
